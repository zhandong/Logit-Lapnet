%% This script is to simulate the PPIov application test 10 times and get the error mean
clear all; 
clc
load PPIOv; %% data generated from the file that contains X, y, Adjacency matrix A--PPIOv
A=Aov;
X=Xov;
y=Yov;
%%------
% lambda_opt=5.3114232864; %% optimal lambda got via CV ===>changed manually
%%------
lambda_opt=15;
alpha1=.2;
...
X1=X1'; 
%% set head and tail xnot respectively and separately
xnot1=X(:,9:83);  
xnot10=X(:,1:72);
 %% Set the rest xnot's which are in the middle
 for i=2:(10-1)
 ...
 for i=1:10
    ...
    %% use x_i 
     ...
     eval(fnw);
      fnw=['[Sp(i) Sen(i)]=GetSpSenY(transpose(y',int2str(i),'),y_hat); '];   %% sensitivity and specificity
      eval(fnw);
     fnw=['logisError(i)=mse(transpose(y',int2str(i),')-y_hat);'];
     eval(fnw);
end
...
save PMSERealPPIov %% save the variables in workspace
