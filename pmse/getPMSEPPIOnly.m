%% This script is to simulate the PPIOnly application test 10 times and get the error mean
clear all; 
clc
load PPIONly; % data generated from the file which contains X, y, Adjacency matrix A
%%-------
lambda_opt=5.3164; % optimal lambda got via CV===> changed manually
%%-------
alpha1=.2;
...
X1=X1'; 
% first have to set the first and the last xnot separately 
xnot1=X(:,9:83);
 xnot10=X(:,1:72);
 %% Set the rest xnot's
 for i=2:(10-1)
...
save PMSERealPPIOnly % save all variables in workspace
